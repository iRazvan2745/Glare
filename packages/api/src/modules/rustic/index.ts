...
function filterSensitiveOptions(options: Record<string, string>): Record<string, string> {
...
function mapRepository(
  ...
  return {
    ...
    options: filterSensitiveOptions(parseOptionsJson(record.optionsJson)),
...
async function proxyToWorker(
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      ...
      return { status: 504, data: { error: "Worker request timed out" } };
    }
    throw error;
  } finally {
...
async function acquireBackupPlanLease(planId: string, leaseMs = PLAN_RUN_LEASE_MS) {
  ...
  const result = await db.execute(sql`
    ...
    returning id
  `);
  const typedResult = result as { rowCount?: number; rows?: unknown[] };
  if (typeof typedResult.rowCount === "number") {
    return typedResult.rowCount > 0;
  }
  return Array.isArray(typedResult.rows) && typedResult.rows.length > 0;
}
