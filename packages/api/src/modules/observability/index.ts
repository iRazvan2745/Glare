  ...
  .get("/api/observability/overview", async ({ request, query, status }) => {
    ...
    const syncEvents =
      workerIds.length > 0
        ...
        : [];
    const syncEventsTruncated = syncEvents.length === SYNC_EVENTS_LIMIT;
    ...
    const requestsInRange = syncEvents.reduce((acc, event) => acc + (event.requestsTotal ?? 0), 0);
    const errorsInRange = syncEvents.reduce((acc, event) => acc + (event.errorTotal ?? 0), 0);
    ...
    return {
      summary: {
        ...
        errorsInRange,
        errorRatePercent:
          requestsInRange > 0 ? Number(((errorsInRange / requestsInRange) * 100).toFixed(2)) : 0,
      },
